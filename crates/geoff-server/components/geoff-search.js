...
class GeoffSearch extends HTMLElement {
  ...
  async _search(query) {
    ...
    try {
      ...
      if (this._store._geoff) {
        const json = this._store.query(sparql);
        arr = JSON.parse(json);
      } else {
        const bindings = this._store.query(sparql);
        arr = (bindings && typeof bindings[Symbol.iterator] === 'function')
          ? [...bindings]
          : bindings;
      }
  ...
  _resolveUrl(row) {
    const url = this._val(row, 'url');
    if (url) return url;
    const s = this._val(row, 's') || '';
  ...
  _renderResults(bindings, query) {
    ...
    for (const row of bindings) {
      const title = this._val(row, 'title') || 'Untitled';
      const url = this._resolveUrl(row);
      if (seen.has(url)) continue;
      seen.add(url);
      const desc = this._val(row, 'desc') || '';
      const date = this._val(row, 'date') || '';
